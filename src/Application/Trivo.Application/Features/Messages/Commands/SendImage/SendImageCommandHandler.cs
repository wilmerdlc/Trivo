...
internal sealed class SendImageCommandHandler(
    ILogger<SendImageCommandHandler> logger,
    ICloudinaryService cloudinaryService,
    IChatRepository chatRepository,
    IMessageRepository messageRepository,
    IRealTimeNotifier notifier
) : ICommandHandler<SendImageCommand, MessageDto>
{
    public async Task<ResultT<MessageDto>> Handle(SendImageCommand request, CancellationToken cancellationToken)
    {
        if (request is null)
        {
        ...
        if (request.Image != null)
        {
            ...
            url = await cloudinaryService.UploadImageAsync(
                stream,
                request.Image.FileName,
                cancellationToken);
        }

        logger.LogInformation("Image uploaded successfully to Cloudinary. URL: {Url}", url);
